...
# 
REPOSITORY="http://git.sv.gnu.org/r/automake.git"
PROJECTS="${HOME}/projects"
CLONE_DIR="${PROJECTS}/automake"
BUILD_DIR="${PROJECTS}/automake_build"
...
#
# remove previous install of automake
#
function clean_previous_install() {
    find ${EXEC_INSTALL} -type f -name 'automake*' -not -newer ${BUILD_DIR}/config.status -exec rm '{}' \;
}

function do_automake_build() {
    mkdir -p ${BUILD_DIR}
    cd ${BUILD_DIR}
...
if [ x${USE_CHECKOUT} != x ]; then
    ...
    if [ ${PACKAGE} ]; then
        ...
        rm -rf ${PROJECTS}/automake.tar.xz
        wget -t0 -c --no-check-certificate ${PACKAGE} -O ${PROJECTS}/automake.tar.xz
        DOWNLOADED=$?
    ...
    if [[ $DOWNLOADED -eq 0 ]]; then
        rm -rf ${CLONE_DIR}
        mkdir -p ${CLONE_DIR}
        xz -dc ${PROJECTS}/automake.tar.xz | tar xf - -C ${CLONE_DIR} --strip-components 1
    else
        exit ${DOWNLOADED}
    fi
fi
